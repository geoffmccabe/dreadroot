...
function BlocksList({ userRoles }: BlocksListProps) {
  ...
  // Block Rain settings
  const [blockRainQuantity, setBlockRainQuantity] = useState<number>(() => {
    const saved = localStorage.getItem('blockRainQuantity');
    return saved ? parseInt(saved) : 100;
  });
  const [blockRainExpiration, setBlockRainExpiration] = useState<number>(() => {
    const saved = localStorage.getItem('blockRainExpiration');
    return saved ? parseInt(saved) : 10;
  });
  const [blockRainSelectedBlocks, setBlockRainSelectedBlocks] = useState<Set<string>>(() => {
    const saved = localStorage.getItem('blockRainSelectedBlocks');
    return saved ? new Set(JSON.parse(saved)) : new Set(['fortress_block', 'grass_block', 'crystal_block']);
  });
  ...
  // Save block rain settings to localStorage
  useEffect(() => {
    localStorage.setItem('blockRainQuantity', blockRainQuantity.toString());
  }, [blockRainQuantity]);

  useEffect(() => {
    localStorage.setItem('blockRainExpiration', blockRainExpiration.toString());
  }, [blockRainExpiration]);

  useEffect(() => {
    localStorage.setItem('blockRainSelectedBlocks', JSON.stringify(Array.from(blockRainSelectedBlocks)));
    // Dispatch event to notify WaterfallFortress of settings change
    window.dispatchEvent(new CustomEvent('blockRainSettingsChanged'));
  }, [blockRainSelectedBlocks]);
