...
function FirstPersonControls({ 
  ...
  // Block placement preview/highlight
  ...
  const calculatePlacementPosition = useCallback((raycaster: THREE.Raycaster) => {
    // Create raycasting targets (same logic as click handler)
    ...
    // Add fortress walls as raycasting targets
    const fortressWalls = [
      { position: [-10, 10, -8], size: [10, 20, 2] },
      { position: [10, 10, -8], size: [10, 20, 2] },
      { position: [-20, 10, -23], size: [2, 20, 30] },
      { position: [20, 10, -23], size: [2, 20, 30] },
      { position: [0, 10, -39], size: [40, 20, 2] }
    ];
    ...
    let placePosition: THREE.Vector3 | null = null;
    ...
    if (intersects.length > 0) {
      ...
      if (normal) {
        ...
        placePosition = hitPoint.clone().add(normal.clone().multiplyScalar(0.5));
      ...
      // Fallback placement at 5 blocks in front of player
      const distance = 5;
      const direction = new THREE.Vector3(0, 0, -1);
      direction.applyQuaternion(camera.quaternion);
      
      placePosition = camera.position.clone().add(direction.multiplyScalar(distance));
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => {
    ...
    // Update block placement preview when in block placement mode
    if (blockPlacementMode && selectedBlockType) {
      const raycaster = new THREE.Raycaster();
      const direction = new THREE.Vector3(0, 0, -1);
      direction.applyQuaternion(camera.quaternion);
      raycaster.set(camera.position, direction);
      
      const newHighlightPosition = calculatePlacementPosition(raycaster);
      setHighlightPosition(newHighlightPosition);
    } else {
      setHighlightPosition(null);
    }
  }, [camera, blockPlacementMode, selectedBlockType, calculatePlacementPosition]);

  // Create highlight mesh for block placement preview
  useEffect(() => {
    if (highlightPosition && blockPlacementMode && selectedBlockType) {
      ...
      if (!highlightMeshRef.current) {
        ...
        scene.add(highlightMeshRef.current);
      }
      
      // Update position
      highlightMeshRef.current.position.copy(highlightPosition);
      highlightMeshRef.current.visible = true;
    } else if (highlightMeshRef.current) {
      highlightMeshRef.current.visible = false;
    }
  }, [highlightPosition, blockPlacementMode, selectedBlockType, scene]);

  // Clean up highlight mesh on unmount
  useEffect(() => {
    return () => {
      if (highlightMeshRef.current) {
        scene.remove(highlightMeshRef.current);
        if (highlightMeshRef.current.geometry) highlightMeshRef.current.geometry.dispose();
        if (highlightMeshRef.current.material) {
          const material = highlightMeshRef.current.material as THREE.Material;
          material.dispose();
        }
  ...
  const handleClick = useCallback(() => {
    ...
    if (!isLocked.current) {
      ...
      // Use the same placement calculation as the preview
      const raycaster = new THREE.Raycaster();
      const direction = new THREE.Vector3(0, 0, -1);
      direction.applyQuaternion(camera.quaternion);
      raycaster.set(camera.position, direction);
      
      const placePosition = calculatePlacementPosition(raycaster);
  ...
  }, [gl, showCrosshairs, onShoot, camera, blockPlacementMode, onBlockPlace, calculatePlacementPosition, selectedBlockType, playAudio, audioRefs]);
