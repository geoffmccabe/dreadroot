...
export const SIEGE_TELEPORTS: SiegeTeleport[] = [
  ...
  // Starblink — the honeycomb land world, its own map. Jump key '@' (next to the '!' sandbox).
  { slot: -2, key: '@', name: 'Starblink (Land)', pos: [0, 3, 0], mapId: 'starblink' },
...
export const SIEGE_DEMOS: SiegeDemoMap[] = [
  ...
  // Positioned directly above HOUSTON (29.7604 N, 95.3698 W) looking at the planet centre, so you
  // arrive over land rather than the empty middle of the Pacific. Derived, not hand-tuned:
  //   dir = (cos(lat)sin(lon), sin(lat), -cos(lat)cos(lon)); pos = dir * 160000; forward = -dir;
  //   yaw = atan2(-fx, -fz) and pitch = asin(fy), the engine's own convention (see FortressControls,
...
export const challengeWorldArrival = (mapId: string | undefined): { pos: [number, number, number]; yaw?: number; pitch?: number } | null => {
  ...
  if (d) return { pos: d.pos, yaw: d.yaw, pitch: d.pitch };
  const t = SIEGE_TELEPORTS.find((x) => x.mapId === mapId);
