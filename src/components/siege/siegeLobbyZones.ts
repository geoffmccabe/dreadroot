...
// the single V key opens whichever zone the player is standing in.
import { useSyncExternalStore } from 'react';

export interface LobbyZone { id: 'vault' | 'market'; x: number; z: number; r: number; }
...
export function setLobbyRanges(v: boolean, m: boolean): void {
  if (v === vaultInRange && m === marketInRange) return;
  vaultInRange = v; marketInRange = m; emit();
}
// Horizontal (x,z) distance test — forgiving for "walk up to it" triggers.
export function rangesAt(x: number, z: number): { v: boolean; m: boolean } {
  let v = false, m = false;
  for (const zn of SIEGE_LOBBY_ZONES) {
    const dx = x - zn.x, dz = z - zn.z;
    const inside = dx * dx + dz * dz < zn.r * zn.r;
    if (zn.id === 'vault') v = inside; else m = inside;
  }
  return { v, m };
}

const snap = () => (vaultInRange ? 1 : 0) | (marketInRange ? 2 : 0);
export function useSiegeLobbyZones(): { vaultInRange: boolean; marketInRange: boolean } {
  const s = useSyncExternalStore((cb) => { subs.add(cb); return () => subs.delete(cb); }, snap, snap);
  return { vaultInRange: !!(s & 1), marketInRange: !!(s & 2) };
}
