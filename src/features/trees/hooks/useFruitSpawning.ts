...
export function useFruitSpawning({
  ...
  // Negative cache: don't re-query trees that failed recently (30s cooldown)
  const failedBlueprintCacheRef = useRef<Map<string, number>>(new Map());
  const BLUEPRINT_FAIL_COOLDOWN_MS = 30 * 1000;

  // Guard against overlapping async ticks
  ...
  const spawnTick = useCallback(async () => {
    ...
    // Prevent overlapping ticks from piling up
    ...
    try {
      ...
      console.log(`[FruitSpawning] Eligible: ${eligibleTrees.length} / ${trees.length} | Rejected: notGrown=${rejectedNotGrown} notOwned=${rejectedNotOwned} noSeedDef=${rejectedNoSeedDef} fungal=${rejectedFungal} noFruiting=${rejectedNoFruiting}`);
      ...
      for (const tree of eligibleTrees) {
        ...
        for (const block of blueprint) {
          ...
          const key = `${fx},${fy},${fz}`;
          if (occupiedPositions.has(key)) { occupiedSkips++; continue; }
        ...
        totalBranches += treeBranches;
      }

      if (missingBlueprints > 0) {
        console.warn(`[FruitSpawning] ${missingBlueprints}/${eligibleTrees.length} eligible trees had NO BLUEPRINT`);
      }
      console.log(`[FruitSpawning] Scanned ${totalBranches} branches across ${eligibleTrees.length - missingBlueprints} trees, ${occupiedSkips} already occupied, ${inserts.length} new fruit to insert`);
