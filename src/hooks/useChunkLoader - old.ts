...
import {
  ...
  PREFETCH_HEADROOM
} from './chunkLoader.constants';
import { ensureBlockCollider, removeBlockCollider } from './chunkLoader.colliders';
import { blocksAreEquivalent } from './chunkLoader.compare';
import { isChunkPinnedImpl, evictLRUChunksImpl } from './chunkLoader.cache';
...
export function useChunkLoader({ worldId, onBlocksChanged }: UseChunkLoaderProps) {
  ...
  const isChunkPinned = useCallback((chunkKey: string): boolean => {
    return isChunkPinnedImpl(chunkKey, playerChunkRef.current, loadedChunksRef.current, UNLOAD_RADIUS);
  }, []);
  ...
  const evictLRUChunks = useCallback(() => {
    evictLRUChunksImpl({
      ...
      isChunkPinned: (key) => isChunkPinnedImpl(key, playerChunkRef.current, loadedChunksRef.current, UNLOAD_RADIUS),
