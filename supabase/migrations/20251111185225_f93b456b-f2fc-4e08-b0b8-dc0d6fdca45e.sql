-- Check and handle existing items table
DO $$ 
BEGIN
...
DROP POLICY IF EXISTS "Superadmins can insert items" ON public.items;
CREATE POLICY "Superadmins can insert items" 
ON public.items 
FOR INSERT 
WITH CHECK (has_role(auth.uid(), 'superadmin'::app_role));

DROP POLICY IF EXISTS "Superadmins can update items" ON public.items;
CREATE POLICY "Superadmins can update items" 
ON public.items 
FOR UPDATE 
USING (has_role(auth.uid(), 'superadmin'::app_role));

DROP POLICY IF EXISTS "Superadmins can delete items" ON public.items;
CREATE POLICY "Superadmins can delete items" 
ON public.items 
FOR DELETE 
USING (has_role(auth.uid(), 'superadmin'::app_role));

-- Migrate existing blocks data to items table
...
FROM public.items
WHERE user_inventory.item_type = items.key AND user_inventory.item_id IS NULL;
