-- Server-Side Tree Growth System
-- Replaces client-side growth with secure, resumable server-side processing
-- Trees grow based on elapsed time since planting, not client ticks

-- Configuration constants (stored in database for easy adjustment)
...
-- Main server-side tree growth function
-- Called by cron job every few seconds to grow all trees
...
BEGIN
  ...
  LOOP
    ...
    -- These are blocks with growthOrder <= expected_orders that aren't in placed_blocks
    FOR v_block IN
      SELECT
        (b->>'x')::INTEGER AS x,
        (b->>'y')::INTEGER AS y,
    ...
    LOOP
      -- Encode block type (format: type_branchDepth_tier)
    ...
    -- Update chunk versions for realtime notifications (only if we inserted blocks)
...
-- Create a helper function to manually trigger growth (useful for testing)
...
-- Grant to authenticated users for manual triggering (admin only in practice)
GRANT EXECUTE ON FUNCTION trigger_tree_growth() TO authenticated;

-- Enable pg_cron extension if not already enabled
-- Note: This requires the extension to be enabled in Supabase dashboard first
-- CREATE EXTENSION IF NOT EXISTS pg_cron;

-- Schedule the growth function to run every 5 seconds
-- Uncomment after enabling pg_cron in Supabase dashboard:
-- SELECT cron.schedule(
--   'tree-growth-processor',
--   '*/5 * * * * *',  -- Every 5 seconds (requires pg_cron with seconds support)
--   'SELECT process_tree_growth()'
-- );

-- Alternative: Schedule every minute if seconds aren't supported
-- SELECT cron.schedule(
--   'tree-growth-processor',
--   '* * * * *',  -- Every minute
--   'SELECT process_tree_growth()'
...
COMMENT ON FUNCTION process_tree_growth() IS
'Server-side tree growth processor. Call periodically via cron or edge function.
Trees grow based on elapsed time since planting - fully resumable after server restarts.
Updates chunk_versions for realtime client notifications.';
