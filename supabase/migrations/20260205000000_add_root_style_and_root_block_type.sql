-- Add root_style column to seed_definitions for buttress root configuration
-- Add 'root' block type support to process_tree_growth() function

-- 1. Add root_style column
ALTER TABLE seed_definitions
ADD COLUMN IF NOT EXISTS root_style TEXT DEFAULT 'none';

COMMENT ON COLUMN seed_definitions.root_style IS 'Style of buttress roots: none, steep, 45deg, shallow';

-- 2. Update process_tree_growth() to support 'root' block type
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Encode block type (includes root and glow_bark)
      ...
      IF COALESCE(v_tree.tree_type, 'original') = 'fungal' THEN
        ...
        END;
      ELSE
        -- Handles both 'original' and 'wide' tree types (same texture routing)
        v_texture_url := CASE v_block.block_type
          ...
          WHEN 'shrine' THEN v_tree.trunk_texture_url
