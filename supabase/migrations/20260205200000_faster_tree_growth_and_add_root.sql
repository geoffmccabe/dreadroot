-- Speed up tree growth by 100x more for testing (from 100x to 10000x total)
-- Also add missing 'root' block type encoding

-- Update speed multiplier to 10000 (was 100)
UPDATE public.tree_growth_config
SET value = 10000, description = 'Speed multiplier when testing_mode is enabled (10000x for fast testing)'
WHERE key = 'speed_multiplier';

-- Update process_tree_growth to handle 'root' block type
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Encode block type (added 'root' and 'glow_bark')
