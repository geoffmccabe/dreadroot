...
create table if not exists public.token_pools (
  ...
  balance         numeric not null default 0,   -- current reserve (funded pools draw this down)
  total_funded    numeric not null default 0,   -- lifetime added (audit)
  total_dispensed numeric not null default 0,   -- lifetime paid out (audit)
  low_water_mark  numeric not null default 0,   -- optional warn threshold
...
-- writes only ever happen through the SECURITY DEFINER functions below (no direct client writes).

-- 2. fund_pool — admin tops a pool up from outside the game (or creates it on first fund).
create or replace function public.fund_pool(
  p_token_theme_id uuid,
  p_amount         numeric,
  p_source         text default 'funded'
) returns json
language plpgsql security definer set search_path = public
as $$
declare v_uid uuid := auth.uid();
begin
  if not (public.has_role(v_uid, 'admin'::app_role) or public.has_role(v_uid, 'superadmin'::app_role)) then
  ...
  on conflict (token_theme_id) do update
    set balance      = public.token_pools.balance + p_amount,
        total_funded = public.token_pools.total_funded + p_amount,
        updated_at   = now();
...
-- 3. grant_currency — now pool-aware. funded pool = hard cap (skip the grant if it can't cover the
--    full amount); minted pool = uncapped game mint (just track dispensed); no pool = legacy mint.
...
begin
  ...
  if v_pool_source = 'funded' then
    -- Draw from the funded reserve; all-or-nothing so dispensed never exceeds funded.
    update token_pools
       set balance = balance - p_amount, total_dispensed = total_dispensed + p_amount, updated_at = now()
    ...
    -- Game-generated (e.g. points): uncapped, just record what was handed out.
    update token_pools set total_dispensed = total_dispensed + p_amount, updated_at = now()
     where token_theme_id = p_token_theme_id;
  end if;
  -- no pool row → legacy behaviour (mint freely, no pool change)
